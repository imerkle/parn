...
const writeTextFile = (file, content, callback = (err) => {
  if (err)
...
const gitIgnoreAdd = () => {
  findUp('.gitignore').then(filepath => {
    readTextFile(filepath, (err, data) => {
      if (!(data.indexOf(BK_FOLDER) > -1)) {
        writeTextFile(filepath, `${data}\n#Add Parn Backup Folder\n${BK_FOLDER}`);
      }
    });
  })
}
const findMixFile = (callback) => {
  println({
    str: 'Finding Mix.exs...',
    type: 'step',
    step: {
...
const noPkError = () => {
  ...
  println({
    str: `${chalk.red("Total Errors: ")} ${chalk.bgRed(packageError.length)}`
  });
  println({
    str: `Done in ${secondsElapsed()}s with ${chalk.redBright(packageError.length)} errors`,
    type: "warn"
  });
}
...
const createMixBackup = () => {
  ...
  const onlyPath = filepath_global.substring(0, filepath_global.lastIndexOf("/")) + `/.${BK_FOLDER}/`;
  spawn(`mkdir -p ${onlyPath}`, (error, stdout, stderr) => {
    writeTextFile(`${onlyPath}mix.exs.${startTime.getTime()}.bk`, mixFileText);
    gitIgnoreAdd();
  });
}
const linkPackages = (callback) => {
  println({
    str: 'Linking packages...',
    type: 'step',
    step: {
      now: 4
    }
...
const executeParn = (obj, callback) => {
  let parnObj = parn(Object.assign({str: mixFileText},obj));
    mixFileText = parnObj.output;
    let modifiedIndex = parnObj.modifiedIndex;
    if (modifiedIndex.length < 1) {
      println({str: `Nothing needed to modify`, type: "warn"});
      footerPrint();
    } else {
      callback(modifiedIndex);
    }
}
const modifiedPackagesPrint = (modifiedIndex, header) => {
  const addedPackages = modifiedIndex.map((o) => {
    return package_names_global[o]
  });
  if (addedPackages.length > 0)
    println({str: chalk.bgMagenta.bold(header)});
  addedPackages.map((o) => {
    println({
      str: chalk.magentaBright(`|_${o}`)
    });
  });
  println({str: "\n"});
}
const pkg_callback = (err, result) => {
  ...
  if (!err && result.version) {
    ...
    if (opts_global.O)
      currentPackage["override"] = true;
    if (suffix == "umbrella") {
      currentPackage["in_umbrella"] = true
    }
    if (opts_global.D) {
  ...
  if (count < package_names_global.length) {
    ...
    if (packageError.length == package_names_global.length) {
      ...
      executeParn({newUpdatedOnes: packageList},(modifiedIndex) => {
        createMixBackup();
        linkPackages(() => {

          println({
            str: 'Downloading packages...',
            type: 'step',
            step: {
              now: 5
            }
          ...
          spawn("mix deps.get", (error, stdout, stderr) => {
            if (stdout)
              println({str: stdout, type: "info"});
            if (error) {
              println({str: error, type: "error"});
              chalkTy = "warn";
            }
            if (err) {
              println({str: err, type: 'error'});
              ...
              if (packageError.length > 0) {
                ...
                if (!error) {
                  modifiedPackagesPrint(modifiedIndex,`Packages Installed`);
                }
                footerPrint(chalkTy);
              }
            }
...
const getPackage = (package_name, callback) => {
  ...
  if (suffix == "umbrella") {
    ...
    let [pkg_name_url,
      exact] = (suffix == "latest" || suffix == "stable" || !suffix)
      ? [pkg_name, false]
      : [`${pkg_name}/${suffix}`, true];
...
const removePackage = (packageNames) => {
  package_names_global = packageNames;

  headingPrint();
  findMixFile(() => {
    executeParn({deleteEmLads: packageNames},(modifiedIndex) => {
      createMixBackup();
      linkPackages(() => {
        modifiedPackagesPrint(modifiedIndex,'Removed Packages');
        hexRunner(`mix deps.clean ${packageNames.join(" ")}`, () => {
          footerPrint();
        });
      });
    });
